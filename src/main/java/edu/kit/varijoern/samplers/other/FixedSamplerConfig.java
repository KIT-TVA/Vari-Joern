...
public class FixedSamplerConfig extends SamplerConfig {
    // Literals used in the TOML file for the configuration of the fixed sampler.
    private static final String FEATURES_FIELD_NAME = "features";

    // Fields for the configuration of the fixed sampler.
        ...
        for (int i = 0; i < configurationsTomlArray.size(); i++) {
            ...
            for (int j = 0; j < featuresTomlArray.size(); j++) {
                try {
                    ...
                    throw new InvalidConfigException(("One of the features of configuration %d for the fixed sampler " +
                            "is not a string.").formatted(i), e
                    );
                }
            }
     ...
     *
     * @return the features ({@link List} of {@link String}s) enabled in each of the configurations. One entry of the
     * top-level {@link List} corresponds to one configuration.
     */
