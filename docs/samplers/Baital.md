...
This sampler returns a sample by choosing configurations using weighted random sampling. Internally, it uses
[Baital](https://github.com/meelgroup/baital) with CMSGen.
To enable statistical analysis, the random number generator used internally is initialized with a random seed.
...
- `rounds`
    ...
    - Default: 10
- `extra-samples`
    - Whether 10x configurations are to be generated per round of which the best are selected.
    - Optional: yes
    - Default: false
